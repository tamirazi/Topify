...
export class SpotifyService {
  ...
  private getMyTopTrack(index: number) {
    this.api('/me/top/tracks?limit=3&time_range=long_term').subscribe(
      (res: TopTracks) => {
        this.getPlaylist(
          [res.items[index].artists[0].id],
          [res.items[index].id],
          []
  ...
  private getMyTopArtist(index: number) {
    this.api('/me/top/artists?limit=3&time_range=long_term').subscribe(
      (res: TopArtists) => {
        this.getPlaylist(
          [res.items[index].id],
          [],
          res.items[0].genres.slice(0, 3)
        ).subscribe((recomendations: any) => {
          this.appData.next({
            result: res.items[index].name,
  ...
  private getMyTopAlbum(index: number) {
    this.api('/me/top/tracks?limit=50&time_range=long_term').subscribe(
      (tracks: TopTracks) => {
        const albumsIds = [];
        ...
        this.api('/albums/' + topThreeAlbums[index][0]).subscribe(
          (res: Album) => {
            this.appData.next({
              result: res.name,
  ...
  private getMyTopGenre(index: number) {
    this.api('/me/top/artists?limit=50&time_range=long_term').subscribe(
      (artists: TopArtists) => {
        const genres = [];
  ...
  private getMyRecentTopTracks(index: number) {
    this.api('/me/player/recently-played?limit=50').subscribe(
      (res: TopTracks) => {
        const tracksId = [];
        ...
        const topTrack = res.items.find(
          (track) => track.track.id === topRecentTracksIds[index][0]
        );

        this.getPlaylist(
          [topTrack.track.artists[0].id],
          [topRecentTracksIds[index][0]],
          []
        ).subscribe((recomendations: any) => {
          this.appData.next({
  ...
  private getMyRecentTopArtist(index: number) {
    this.api('/me/player/recently-played?limit=50').subscribe(
      (res: TopTracks) => {
        ...
        const topTrack = res.items.find(
          (track) => track.track.artists[0].id === topRecentArtistIds[index][0]
        );
      ...
      (res: TopTracks) => {
        ...
        this.api('/albums?ids=' + topRecentAlbumsIds[index][0])
          ...
          .subscribe((albums: Album[]) => {
            const topAlbum: Album = albums.find(
              (album) => album.id === topRecentAlbumsIds[index][0]
            );
            this.appData.next(
              new AppDataObject(
  ...
  private getMyRecentTopGenre(index: number) {
    this.api('/me/player/recently-played?limit=50').subscribe(
      (res: TopTracks) => {
        ...
        const topRecentArtistIds = this.getTopThreeFromArray(artistsIds);
        this.api('/artists/' + topRecentArtistIds[index][0]).subscribe(
          (artist: Artist) => {
            this.getPlaylist(
              [artist.id],
              [],
  ...
  createPlaylistFromTracks(
    ...
    this.http
      .post(
        environment.API_URL + '/users/' + userId + '/playlists',
        body,
        httpOptions
      )
      .subscribe(
        (res: any) => {
          ...
          this.http
            .post(res.tracks.href, addTracksBody, httpOptions)
